...
import { renderHostMessage, type HostMessageParams } from '../../shared/host-messages.ts';
...
export type StaticTranslationKey = {
  [K in TranslationKey]: Variables<Text<K>> extends never ? K : never;
}[TranslationKey];
...
export function createLocale(locale: Locale, production = false) {
  ...
  // These titles are fixed host UI metadata; activity details remain verbatim.
  const activityTitle = (title: string) => {
    ...
    const key = keys.map((name) => `activity.${name}` as const).find((key) => zhCN[key] === title);
    if (key) return t(key);
    const prefix = zhCN['activity.tool'].split('{name}')[0];
    return title.startsWith(prefix)
      ? t('activity.tool', { name: title.slice(prefix.length) })
      : title;
  };
  const host = (legacy: string, descriptor?: MessageDescriptor) =>
    descriptor
      ? renderHostMessage(
          { code: descriptor.code, params: (descriptor.params ?? {}) as HostMessageParams, legacy },
          locale,
        )
      : legacy;
  return { t, number, date, time, duration, activityTitle, host };
}
